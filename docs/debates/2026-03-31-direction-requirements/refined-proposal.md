...
| Epidemic Sound契約 | 回答待ち | Module 5 BGM検索の精度・利便性 |
| 優先Module | 回答待ち | 開発着手順序 |
...
#### 3.2.1 機能概要
動画の無音部分、裏方スタッフの声が入ったシーンを自動検出し、**統合JSON中間フォーマット**として出力する。Premiere Proへの反映はJSON経由で一括適用する。

#### 3.2.2 機能要件

| ID | 要件 | 優先度 |
|----|------|-------|
| M2-01 | 無音区間（閾値調整可能、デフォルト2秒以上）を自動検出する | 必須 |
...
| 出力 | **統合JSON中間フォーマット**（無音区間・話者分離・マーカー含む）、Premiere Pro一括適用スクリプト |
...
> **討論結果:** 完全自動画面録画（Playwright + FFmpeg gdigrab）は、DRM（Widevine）による黒画面リスク・無人運用の不安定性・法的リスクにより、**半自動方式に格下げ**。手動録画+自動解析をメインフローとする。

| ID | 要件 | 優先度 |
|----|------|-------|
| M3B-01 | **Tier 1（自動・推奨）:** YouTube公式チャンネルからyt-dlpで自動ダウンロードし、解析パイプラインに投入する | 必須 |
| M3B-02 | **Tier 2（半自動・メイン）:** ユーザーが手動でOBS録画した動画ファイルを指定フォルダに配置すると、自動解析パイプラインが検出して処理する | 必須 |
| M3B-03 | **Tier 3（オプション）:** HDMIキャプチャデバイス経由の取り込み（DRM回避可能な環境のみ、自己責任） | 任意 |
| M3B-04 | 取り込みファイルをMP4形式でローカル保存する | 必須 |

**DRMリスク・法的注意事項:**
- TVer/ABEMAはWidevine DRMを使用しており、ハードウェアアクセラレーション有効時は映像が保護され録画不可
- ハードウェアアクセラレーション無効化+ソフトウェアレンダリングで録画可能な場合があるが品質低下は避けられない
- 各サービスの利用規約では録画を禁止している（私的使用目的のみ・外部共有不可）
- HDCPハンドシェイクエラー、解像度不一致等の物理エラーが発生し得る
- 将来的に安定した自動録画手段が確立された場合、完全自動に昇格を検討
...
| Tier 2自動検出 | 指定フォルダ配置後60秒以内に解析パイプラインが起動する |
...
#### 3.7.1 機能概要
Google Driveの動画素材を夜間に自動でローカルPCにダウンロードする。
...
| M7-03 | Windows Task Schedulerで毎日深夜2時に自動実行する | 必須 |
...
| `/カット [動画パス]` | Module 2 | 無音カット + 話者分離 → JSON出力 → Premiere Pro一括適用 |
| `/参考検索 "[イメージ]"` | Module 3-A | YouTube/Vimeo/TwelveLabs検索（リファレンスチャンネル限定可） |
| `/参考取り込み [フォルダパス]` | Module 3-B | 手動録画ファイルの自動解析・蓄積 |
...
| ローカル処理 | PC起動中のみ（夜間同期はTask Scheduler） |
...
CREATE TABLE reference_metadata (
  ...
  channel TEXT,
  source_url TEXT,
  tier TEXT CHECK (tier IN ('youtube', 'manual_obs', 'hdmi_capture')),
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
| 参考動画録画（手動OBS） | 20〜50 GB | 週5本×1時間×2〜5GB |
...
| タスク | 内容 | 担当 |
|--------|------|------|
| P1-01 | auto-editor導入・**JSON中間フォーマット出力テスト** | 開発 |
| P1-02 | rclone + Task Scheduler 夜間同期設定 | 開発 |
| P1-03 | kotoba-whisper環境構築・文字起こしテスト | 開発 |
| P1-04 | **SRT→Premiere Proキャプショントラック流し込みテスト** | 開発 |
...
| タスク | 内容 | 担当 |
|--------|------|------|
| P2-01 | n8n(Hostinger) Google Drive→Whisper→pgvectorパイプライン構築 | 開発 |
| P2-02 | **pgvectorスキーマ構築・楽観的ロック実装** | 開発 |
| P2-03 | **MOGRTテンプレート作成（AE）+ ExtendScriptテキスト差し替え** | 開発 |
| P2-04 | Premiere Pro ExtendScript **JSON一括適用**スクリプト開発 | 開発 |
| P2-05 | PySceneDetect シーン自動分割 + **CLIPクラスタリング + 4x4グリッド生成** | 開発 |
| P2-06 | pgvector セマンティック検索インターフェース構築 | 開発 |
| P2-07 | **WhisperX + pyannote話者分離 + LLMテキスト文脈判定ハイブリッド実装** | 開発 |

### Phase 3: 2〜4週間（高度機能）

| タスク | 内容 | 担当 |
|--------|------|------|
| P3-01 | **手動OBS録画ファイル自動検出→解析パイプライン構築** | 開発 |
| P3-02 | 録画シーン分割・タグ付け・蓄積システム構築 | 開発 |
| P3-03 | BGM横断検索（壁打ちモード + **参考曲類似検索モード**）構築 | 開発 |
| P3-04 | 参考動画ライブラリ壁打ち検索機能構築 | 開発 |
| P3-05 | **Premiere Pro UXPパネル開発開始（Module 2/4優先）** | 開発 |
...
| P4-03 | 参考動画ライブラリの蓄積・活用最適化 | 開発+制作 |
...
| TVer/ABEMAのDRM（Widevine）による録画不可 | 高 | 半自動方式（手動OBS録画+自動解析）に格下げ済み |
| TVer/ABEMAの利用規約変更 | 低 | 録画機能を停止可能な設計 |
...
> 以下は討論で方向性は定まったが、最終判断にユーザー確認が必要な事項。

### 11.1 UX方式の最終確定

| 選択肢 | メリット | デメリット |
|--------|---------|----------|
| **現採用: 2層（Claude Code→UXP）** | 開発コスト抑制、早期稼働可能 | Phase 1〜2はチャットUIで操作（動画プレビュー不可） |
| Gemini案: 最初からUXP一本化 | 動画編集者にとって最適なUX | 開発期間2〜3ヶ月追加、Phase 1稼働が遅延 |

**確認事項:** Phase 1〜2のClaude Code（チャットUI）での運用を許容できるか？

### 11.2 Module 3-B の将来方針

| 選択肢 | メリット | デメリット |
|--------|---------|----------|
| **現採用: 半自動（手動録画+自動解析）** | ユーザー要件を維持、法的リスク低減 | 録画作業が手動のまま |
| Gemini案: 完全削除（Video-lessリサーチ） | 法的リスクゼロ、開発工数削減 | TVer/ABEMAの映像参考が不可能に |

**確認事項:** 手動OBS録画の運用負荷は許容範囲内か？

### 11.3 アーキテクチャの将来スケール

| 選択肢 | メリット | デメリット |
|--------|---------|----------|
| **現採用: pgvector共有+楽観的ロック** | シンプル、現在の規模に適切 | 大規模スケール時にRace Conditionリスク |
| Gemini案: Pub/Sub+ローカルインデックス | フォールトトレラント、スケーラブル | 現在の規模にはオーバーエンジニアリング |

**確認事項:** 将来的にチーム規模・処理量が大幅に増加する見込みはあるか？

### 11.4 システム全体の死活監視

Geminiが追加指摘した「n8n自体のダウン検知・アラート機構」について。Hostinger上のn8nがダウンした場合の検知・自動復旧手段を検討する必要がある。
...
4. Module 3-B: 完全自動録画→半自動（手動録画+自動解析）に格下げ、DRMリスク明記
5. UX: 2層（Claude Code→UXP）に変更。Web中間層を削除
6. 話者分離: レギュラー声紋+ゲストLLM文脈判定のハイブリッド
7. BGM検索: 壁打ちモード+参考曲URL類似検索の2系統
8. pgvectorスキーマ設計セクション追加（テーブル定義・インデックス・楽観的ロック）
9. 受入テスト基準を各Moduleに追加
10. エラーハンドリング共通方針を追加
11. ストレージ計画を追加（月150〜600GB増加想定）
12. VFR（可変フレームレート）対策を追加
13. サービスアカウント最小権限設定を追加
14. アーキテクチャ: 共有キャッシュ型+楽観的ロック
15. YouTube検索: リファレンスチャンネル限定検索追加
