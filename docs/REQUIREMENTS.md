...
| テロップ制作 | Adobe Photoshop | 既契約（Adobe CC）、PSDテンプレート+テキスト中心 |
| クラウドストレージ | Google Drive | サービスアカウント接続 |
...
| M1-04 | 各シーンの代表フレーム画像をAI（Gemini Flash/Claude Vision）で分析し、背景・画角・内容を分類する | 必須 |
...
| 項目 | 内容 |
|------|------|
| 入力 | Google DriveフォルダパスまたはローカルフォルダパスAS |
| 出力 | シーン分割済み動画クリップ、CSVレポート、代表フレーム画像、DBレコード |
...
| 精密補完 | TransNetV2 |
| メタデータ抽出 | FFprobe |
| フレーム分析 | Gemini Flash API / agentic-vision |
...
#### 3.2.1 機能概要
動画の無音部分、裏方スタッフの声が入ったシーンを自動検出し、Premiere Proのタイムラインとして出力する。
...
| M2-02 | 検出結果をPremiere Pro互換のタイムラインXMLとして出力する | 必須 |
| M2-03 | WhisperXで音声の文字起こし+話者分離を行う | 必須 |
| M2-04 | 話者ごとにラベル（SPEAKER_00等）を付与する | 必須 |
| M2-05 | 出演者の声を事前登録し、未登録話者をスタッフ候補として識別する（pyannoteAI Voiceprint） | 推奨 |
| M2-06 | 手動ラベリングによる話者識別（初回設定）をサポートする | 必須 |
| M2-07 | スタッフ該当セグメントをPremiere Proマーカーとして出力する | 必須 |
| M2-08 | 「よーい、スタート」「カット」等のキーワードを転写テキストから検出する | 推奨 |
...
| 出力 | Premiere Pro タイムラインXML、話者分離レポート、マーカーリスト |

#### 3.2.4 使用技術

| コンポーネント | 技術 |
|---|---|
| 無音カット | auto-editor v30 (--export premiere) |
| 文字起こし+話者分離 | WhisperX + pyannote 3.1 |
| 日本語文字起こし | kotoba-whisper v2.2 |
| 話者事前登録 | pyannoteAI Voiceprint (€19/月) |
| 音声処理 | FFmpeg 8.0 |
| NLE連携 | Premiere Pro ExtendScript/UXP |
...
**3-B: TVer/ABEMA 自動画面録画**

| ID | 要件 | 優先度 |
|----|------|-------|
| M3B-01 | Playwrightでブラウザを自動操作し、指定URLの番組ページに遷移する | 必須 |
| M3B-02 | 再生ボタンクリック・全画面化を自動実行する | 必須 |
| M3B-03 | FFmpeg gdigrabでデスクトップ画面をリアルタイム録画する | 必須 |
| M3B-04 | 音声も同時録画する（DirectShow Stereo Mix） | 必須 |
| M3B-05 | 時間指定または番組終了検知で録画を自動停止する | 必須 |
| M3B-06 | 録画ファイルをMP4形式でローカル保存する | 必須 |
| M3B-07 | スケジュール録画（夜間自動）をサポートする | 推奨 |
...
| M3C-02 | 各シーンの代表フレームをCLIP/Gemini Visionでベクトル化する | 必須 |
...
#### 3.3.3 制約事項
- 画面録画中はPC画面が専有される（別モニターまたは仮想デスクトップで回避可能）
- TVer/ABEMAの録画は私的使用（著作権法第30条）の範囲内。外部共有・二次利用は不可
- 各サービスの利用規約では録画を禁止している点は認識の上で利用
...
#### 3.4.1 機能概要
動画の文字起こしから、シーンに合ったテロップを自動作成し、Photoshopでデザイン→Premiere Proに配置する。
...
| M4-07 | **Photoshop ExtendScriptでPSDテンプレートにテキストを自動流し込みする** | 必須 |
| M4-08 | テロップ画像をPNG（透過背景）として一括書き出しする | 必須 |
| M4-09 | 書き出したPNGをPremiere Proタイムラインに自動配置する | 必須 |
| M4-10 | Premiere Pro上でテロップの位置・タイミングを手動修正可能にする | 必須 |
...
```
[Phase 1] 文字起こし
  kotoba-whisper v2.2 → タイムスタンプ付きJSON/SRT

[Phase 2] テロップ候補提案
  Claude Code 壁打ち → テキスト候補 + スタイル方向性

[Phase 3] テロップデザイン生成
  Photoshop ExtendScript → PSDテンプレートにテキスト流し込み
  → PNG書き出し（透過背景）

[Phase 4] Premiere Pro配置
  ExtendScript/UXP → タイムライン上に自動配置
  → 手動修正可能
```
...
| テロップ壁打ち | Claude Code |
| テロップデザイン | **Photoshop ExtendScript** (PSDテンプレート + テキスト流し込み) |
| 画像書き出し | Photoshop PNG書き出し（透過背景） |
...
#### 3.5.1 機能概要
Claude Codeとの壁打ちで音のイメージを構造化し、複数サイトを横断検索して候補を提示する。

#### 3.5.2 機能要件

| ID | 要件 | 優先度 |
|----|------|-------|
| M5-01 | Claude Codeとの壁打ちで音のイメージを構造化する（ムード/楽器/BPM/ボーカル/尺） | 必須 |
...
| M6-08 | PySceneDetectでシーン分割し、代表フレームをCLIPベクトル化する | 推奨 |
...
#### 3.8.1 機能概要
全ModuleをClaude Code上のスキルコマンドとして統合操作する。
...
| `/カット [動画パス]` | Module 2 | 無音カット + スタッフ声マーク → Premiere Pro出力 |
| `/参考検索 "[イメージ]"` | Module 3-A | YouTube/Vimeo/TwelveLabs検索 |
| `/参考録画 [URL]` | Module 3-B | TVer/ABEMA画面録画 → 解析・蓄積 |
| `/参考ライブラリ "[検索クエリ]"` | Module 3-D | 蓄積した参考動画の壁打ち検索 |
| `/テロップ [動画パス]` | Module 4 | 文字起こし → テロップ候補 → Photoshop → Premiere Pro |
| `/BGM検索 "[イメージ]"` | Module 5 | Freesound + Jamendo + Epidemic Sound 横断検索 |
...
| タスク | 内容 | 担当 |
|--------|------|------|
| P1-01 | auto-editor導入・Premiere Pro出力テスト | 開発 |
...
| P2-02 | Photoshop ExtendScript テロップ流し込みスクリプト開発 | 開発 |
| P2-03 | Premiere Pro ExtendScript テロップ配置スクリプト開発 | 開発 |
| P2-04 | PySceneDetect シーン自動分割パイプライン構築 | 開発 |
| P2-05 | pgvector セマンティック検索インターフェース構築 | 開発 |

### Phase 3: 2〜4週間（高度機能）

| タスク | 内容 | 担当 |
|--------|------|------|
| P3-01 | Playwright + FFmpeg 画面録画パイプライン構築 | 開発 |
| P3-02 | 録画シーン分割・タグ付け・蓄積システム構築 | 開発 |
| P3-03 | WhisperX + pyannote 話者分離統合 | 開発 |
| P3-04 | BGM横断検索（Freesound + Jamendo + Epidemic Sound MCP）構築 | 開発 |
| P3-05 | 参考動画ライブラリ壁打ち検索機能構築 | 開発 |
...
| P4-02 | テロップPSDテンプレートライブラリ構築（TV番組ジャンル別） | 開発+制作 |
| P4-03 | 参考動画ライブラリの蓄積・活用最適化 | 開発+制作 |
| P4-04 | 全体テスト・フィードバック反映・運用ドキュメント作成 | 開発+制作 |
