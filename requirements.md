...
| 文字起こしエンジン | Gemini 3.1 Flash-Lite Preview | タイムスタンプ精度最安定、低コスト |
...
- GPU非搭載の場合：クラウドAPI（Gemini API $0.006/分等）で代替。月$20〜40追加
...
[素材取り込み]
  ...
  ↓ Gemini 3.1 Flash-Lite Preview文字起こし
  ↓ PySceneDetectシーン分割
  ↓ 結果をpgvector DBに格納
  ↓
...
| 日本語文字起こし | Gemini 3.1 Flash-Lite Preview |
...
| M3C-03 | Gemini 3.1 Flash-Lite Previewでシーンごとの文字起こしを行う | 必須 |
...
| ID | 要件 | 優先度 |
|----|------|-------|
| M4-01 | Gemini 3.1 Flash-Lite Previewで動画音声の日本語文字起こしを行う | 必須 |
...
[Phase 1: 即時運用（SRTキャプション）]
  Gemini 3.1 Flash-Lite Preview → タイムスタンプ付きSRT
...
| コンポーネント | 技術 |
|---|---|
| 文字起こし | Gemini 3.1 Flash-Lite Preview |
| テロップ壁打ち | Claude Code |
...
| M6-03 | Gemini 3.1 Flash-Lite Preview（またはGemini API）で文字起こしする | 必須 |
...
| 音声抽出 | FFmpeg |
| VFR→CFR変換 | FFmpeg (-vsync cfr) |
| 文字起こし | Gemini 3.1 Flash-Lite Preview / Gemini API |
| ベクトル化 | Gemini gemini-embedding-001 (3072次元) |
...
| Gemini 3.1 Flash-Lite Previewバージョン管理 | pip/condaで管理、メジャー更新時に精度検証 |
...
CREATE TABLE transcripts (
  ...
  text TEXT NOT NULL,
  text_embedding vector(1536),   -- Gemini gemini-embedding-001 (3072次元)
  start_tc TEXT NOT NULL,
  end_tc TEXT NOT NULL,
  speaker_id TEXT,
...
| Gemini API | 文字起こし（GPU非搭載時） | APIキー | $0.006/分 |
| Gemini Embedding API | ベクトル検索（gemini-embedding-001） | APIキー | 低コスト |
...
| P1-03 | Gemini 3.1 Flash-Lite Preview環境構築・文字起こしテスト | 開発 |
...
15. 文字起こしエンジン: kotoba-whisper / OpenAI Whisper API → **Gemini 3.1 Flash-Lite Preview** に変更（タイムスタンプ精度最安定）
16. Embedding: OpenAI text-embedding-3-small → **Gemini gemini-embedding-001 (3072次元)** に変更
17. サムネイル機能を追加（FFmpegでタイムコードのフレーム画像をBase64でDB保存、検索結果に表示）
18. Web検索画面をn8n Webhookで構築（フォルダ選択ドロップダウン付き、誰でもブラウザからアクセス可能）
19. 大規模バッチ処理スクリプト（bulk_runner.py）を追加。Hostinger VPSで独立実行、再開機能付き
20. 共有ドライブ（Team Drive）対応を追加（supportsAllDrives）

---

## 実装タスク

### Phase 1: Module 6（過去動画素材の掘り起こし）
- [x] pgvectorスキーマ構築（Neon Serverless）
- [x] 動画処理パイプライン（FFmpeg音声抽出・文字起こし・ベクトル化・DB登録）
- [x] セマンティック検索スクリプト（キーワード + ベクトル類似度のハイブリッド）
- [x] Google Drive連携（サービスアカウント・共有ドライブ対応）
- [x] SRT/JSONファイルの自動生成・Google Driveアップロード
- [x] Gemini 3.1 Flash-Lite Previewへの移行（タイムスタンプ精度改善）
- [x] Gemini gemini-embedding-001への移行
- [x] サムネイル生成機能（FFmpegフレーム抽出・Base64・DB保存）
- [x] Web検索画面（n8n Webhook・フォルダ選択ドロップダウン・サムネイル表示）
- [x] 大規模バッチ処理スクリプト（Hostinger VPS実行・再開機能・並列処理）
- [x] 日本語ファイル名のASCIIエンコードエラー修正
- [ ] 全対象フォルダの動画処理完了（6,506本中、処理進行中）
- [ ] サムネイルなし既存データへのサムネイル一括生成
- [ ] タイムスタンプ尺超過データ（60件）の修正
- [ ] 処理スキップ（NO SPEECH）のDB記録対応

### Phase 2: Module 7（自動ダウンロード）
- [ ] rclone設定（Google Drive サービスアカウント認証）
- [ ] 電源接続トリガー設定（macOS: sleepwatcher/pmset、Windows: Task Scheduler）
- [ ] n8n Webhook通知（完了/エラー）
- [ ] 同期完了後の再スリープ

### Phase 3: Module 3（参考動画リサーチ・録画・解析）
- [ ] 参考動画検索（YouTube API + 壁打ち）
- [ ] TVer/ABEMA自動画面録画（Playwright + OBS）
- [ ] 録画のシーン分割・タグ付け・蓄積
- [ ] 蓄積した参考動画の壁打ち検索
